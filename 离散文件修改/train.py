...
def main(args: argparse.Namespace):
    ...
    def move_to_device(example_batch):
        return {k: v.to(device) if isinstance(v, torch.Tensor) else v for k, v in example_batch.items()}
    ...
    tokenizer = AutoTokenizer.from_pretrained("./ModelBin")
    ...
    train_dataset = train_dataset.map(move_to_device)
    if test_dataset is not None:
        test_dataset = test_dataset.map(move_to_device)
    ...
    accuracy = evaluate.load("evaluate-main/evaluate-main/metrics/accuracy")
    ...
    model = AutoModelForSequenceClassification.from_pretrained("./ModelBin", num_labels=2)
    ...
    training_args = TrainingArguments(
        ...
        eval_steps=1000 if 'sent' in args.input else 500,
        save_steps=500,  # 例如，每500步保存一次
        save_total_limit=20  # 例如，最多保存3个模型检查点
    )
