...
def predict_data(setting, lang, checkpoint, device, batch_size):
    if '-pair' in checkpoint:
        ...
        def func(batch):
            out = detector(inputs=batch['answer'], max_length=512, truncation=True)
            batch['pred'] = [int(o['label'][-1]) for o in out]
            # batch['prob'] = [o['score'] for o in out]
            return batch

    path = f"evaluate_data/test_test.csv"  # path to the csv data from the google drive
    print('\n\n', path)
...
def main(args: argparse.Namespace):
    ...
    model_prefix = './models/all/zh/mix/' + args.input
    if args.pair:
        model_prefix += '-pair'
    ...
    if args.multisource:
        ...
        # printf(ckpt)
        ckpt = "./models/all/zh/mix/checkpoint-11500"
